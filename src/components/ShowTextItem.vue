<template>
  ...
  >
    <router-link :to="`/passage/${data.id}`" >
      <div class="cover">
        <Cover :source="data.commentImg" style="height: 200px;" fit="cover"></Cover>
        <div class="shade">
    ...
    <div class="video-info">
      <router-link 
        class="title" :to="`/passage/${data.id}`" 
        v-html="data.title"
        target="_blank"
      ></router-link>
      <router-link 
      ...
      >
        <!-- 发布者的名字 -->
        <span class="iconfont icon-yonghu1">{{ PostUserInfo.username }}</span>
        <!-- 发布的时间 -->
...
const getVideoWriter = async(PostUserId)=>{
  console.log("开始获取文章发布者信息"+PostUserId)
  let res = await getPassageWritterMS(PostUserId);
  if(res.code == 200){
...
.video-item{
  ...
  &:hover {
    ...
    box-shadow: 0 0 20px rgba(255, 255, 255, 0.6), 
                0 0 40px rgba(64, 196, 119, 0.61); /* 发亮光效 */
